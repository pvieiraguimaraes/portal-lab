...
public class RoleVoter extends BaseVoter{
	...
	public int vote(Authentication authentication, Object object, Collection<ConfigAttribute> attributes) {
		...
		if (useCaseFunctionality != null && useCaseFunctionality.length == 2) {
			for (GrantedAuthority grantedAuthority : authentication.getAuthorities()) {
				granted = isGrantedPermission(grantedAuthority.getAuthority(), useCaseFunctionality[0], useCaseFunctionality[1]);
				if(granted) break;
			}
		}
		if(granted){
	...
	private Boolean isGrantedPermission(String profile, String useCase, String functionality) {
		return getSpringSecurityManager().isGrantedFunctionality(profile, useCase, functionality);
	}
