...
insert into grupo_permissao(id_grup_grpe,id_cafu_grpe)
...
where not exists(
	...
	where		p2.id_grup_grpe = grpe.id_grup
			and p2.id_cafu_grpe = grpe.id_cafu
)

insert into usuario(login,senha,status,nome)
select 
...
from(
	select
		(select id_usuario from usuario where login='SEGADMIN') as id_usua_usgr,
