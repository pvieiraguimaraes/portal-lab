...
public class CrossWordValidatorControl extends ValidatorControl{
	public CrossWordValidatorControl(MessagesControl pMessagesControl, int orderValidate, List<String> actionsToValidate) {
		super(pMessagesControl, orderValidate, actionsToValidate);
	}
	public CrossWordValidatorControl(MessagesControl pMessagesControl, int orderValidate) {
	...
	public boolean doAction(HashMap<String, Object> mapFields, String action) {
		if(action.equalsIgnoreCase("validateAnswer"))
		{
			if (validarTamanhoAnswer(mapFields))
			{
				...
				{
					this.messagesControl.addMessageError("interseccao_resposta");
					return false;
				}
			}
			else
	...
	private boolean validarInterceccaoAnswer(HashMap<String, Object> mapFields) {
		...
		HashMap<String, Square> squares = (HashMap<String, Square>) mapFields.get("squares");
		for(int i=0;i<answer.getValue().length();i++)
		{
			...
			if(square != null)
			{
				...
				if (answer.getType() == TypeAnswer.HORIZONTAL)
				{
					if (square.getHorizontalAnswer() != null && square.getHorizontalAnswer() != answer)
					{
				...
				/*
				 * Validação  a ser pensada
				//Existe caso q essa validação não é bom
				if (square.getHorizontalAnswer() != null && square.getVerticalAnswer() != null)
				{
					return false;
				}
				*/
	...
	private boolean validarInterceccaoSquare(Square square, Answer answer, Integer i) {
		if (square.getHorizontalAnswer() != null || square.getVerticalAnswer() != null)
		{
			...
			if (square.getValue() != null &&
				!(  square.getValue().equalsIgnoreCase(answer.getValue().substring( letterPosition - 1,letterPosition))) 
				)
				return false;
		}
		return true;
	...
	private boolean validarTamanhoAnswer(HashMap<String, Object> mapFields) {
		Answer answer = (Answer) mapFields.get("actualAnswer");
		Integer xCrossWordLength = ((CrossWord) mapFields.get("entity")).getCrossXDimension();
		Integer yCrossWordLength = ((CrossWord) mapFields.get("entity")).getCrossYDimension();
		if (answer.getType() == TypeAnswer.HORIZONTAL) //x
		{
			if(answer.getValue().length() > xCrossWordLength - (answer.getStartPositionX() - 1))
			{
			  return false;
			} 
		}
		...
		{
			if(answer.getValue().length() > yCrossWordLength - (answer.getStartPositionY() - 1))
			{
			  return false;
			}
		}
