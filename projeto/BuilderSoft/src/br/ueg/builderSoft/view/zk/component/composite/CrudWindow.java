...
public class CrudWindow extends Window implements IFormWindow{
	...
	/**
	 *  Atributo para indicar quais operações estaram disponível no formulário de crud
	 *  Valor padrão é "create,read,update,delete" ou seja todas as operações disponíveis
	 */
	...
	private void initCrudWindow(ComposerController<?> composer) {
		...
		//inicializar as variaveis de controle de operação baseado no atributo crudOperation
		initCrudOpeartion();
	...
	/**
	 * faz a construção dos itens(listitem) da janela, pegando o conteudo da div listitem 
	 * (que está na definição da janela utilizando o componente crudWindow) e adiciona 
	...
	/**
	 * faz a construção do cabeçalho(listheader) da janela, pegando o conteudo da div listhead 
	 * (que está na definição da janela utilizando o componente crudWindow) e adiciona 
	 * na jenela gerada pelo componente
	 * @param composer
	 */
	...
	/**	 * 
	 * @return the crudOperation , que é uma string separada por virgula que pode ter:
	 * create - para mostrar o botão de novo
	 * read - para mostrar partes de leitura
	 * update - para mostrar o botão de editar
