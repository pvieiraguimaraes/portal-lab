...
import br.ueg.portalLab.control.GenericControl;
import br.ueg.portalLab.model.Entity;
import br.ueg.portalLab.util.annotation.AttributeView;
import br.ueg.portalLab.util.constant.ControllerType;
import br.ueg.portalLab.util.control.ListingControl;
import br.ueg.portalLab.util.reflection.Reflection;
import br.ueg.portalLab.view.control.MessagesWeb;
...
public abstract class MB<E extends Entity> {
	...
	/**
	 * Método que verifica se a ação foi de listagem ou se já está sendo listado as entidades
	 * se positivo, recebe/atualiza a lista
	 */
	...
	public List<ManagedBeanField> getListColumns() {
		...
		for (Class<?> reflectedClass = this.getClass(); reflectedClass != null; reflectedClass = reflectedClass.getSuperclass()) {
			...
			for (Field field : fields) {
				if (field.getAnnotation(br.ueg.portalLab.util.annotation.AttributeView.class) != null) {
					try {
						//Object fieldValue = Reflection.getFieldValue(this, field.getName());
						String vKey = field.getAnnotation(br.ueg.portalLab.util.annotation.AttributeView.class).key();
						String fieldName = field.getName();
						//Class fieldType = field.getAnnotation(br.ueg.portalLab.util.annotation.AttributeView.class).entityType();
						String vCaption = field.getAnnotation(br.ueg.portalLab.util.annotation.AttributeView.class).caption();
						String vEntityType = Reflection.getClassName(field.getAnnotation(br.ueg.portalLab.util.annotation.AttributeView.class).entityType());
						boolean vIsVisible = field.getAnnotation(br.ueg.portalLab.util.annotation.AttributeView.class).isVisible();
						int vComponent = field.getAnnotation(br.ueg.portalLab.util.annotation.AttributeView.class).component();
						if (field.getAnnotation(br.ueg.portalLab.util.annotation.AttributeView.class).isEntityValue()) {
